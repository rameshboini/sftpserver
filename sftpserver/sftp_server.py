...
class OperationTracker:
    """Tracks SFTP operations for testing and verification."""

    def __init__(self):
        self._operations: list[dict] = []
        self._lock = threading.Lock()

    def record(self, operation: str, path: str, success: bool, error: str | None = None) -> None:
        """Record an SFTP operation."""
        with self._lock:
    ...
    def get_operations(self) -> list[dict]:
        """Get all recorded operations (copy)."""
        with self._lock:
...
class LocalSFTPServer(SFTPServerInterface):
    def __init__(self, server, *args, tracker: OperationTracker | None = None, **kwargs):
    ...
    def _record_op(self, operation: str, path: str, success: bool, error: str | None = None) -> None:
        if self.tracker:
            self.tracker.record(operation, path, success, error)
    ...
    def list_folder(self, path: str):
        try:
            ...
            for entry in os.scandir(local):
                ...
                attrs.append(attr)
            self._record_op("list", path, True)
            return attrs
        except PermissionError:
            self._record_op("list", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("list", path, False, str(exc))
            return os_error_to_sftp(exc)

    def stat(self, path: str):
        try:
            result = SFTPAttributes.from_stat(os.stat(self._local(path)))
            self._record_op("stat", path, True)
            return result
        except PermissionError:
            self._record_op("stat", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("stat", path, False, str(exc))
            return os_error_to_sftp(exc)

    def lstat(self, path: str):
        try:
            result = SFTPAttributes.from_stat(
                os.lstat(self._local(path, follow_symlinks=False))
            )
            self._record_op("lstat", path, True)
            return result
        except PermissionError:
            self._record_op("lstat", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("lstat", path, False, str(exc))
            return os_error_to_sftp(exc)
    ...
    def open(self, path: str, flags: int, attr: SFTPAttributes):
        try:
            ...
            self._record_op("open", path, True)
            return handle
        except PermissionError:
            self._record_op("open", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("open", path, False, str(exc))
            return os_error_to_sftp(exc)

    def remove(self, path: str) -> int:
        try:
            os.remove(self._local(path, follow_symlinks=False))
            self._record_op("remove", path, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("remove", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("remove", path, False, str(exc))
            return os_error_to_sftp(exc)
    ...
    def rename(self, oldpath: str, newpath: str) -> int:
        try:
            ...
            self._record_op("rename", oldpath, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("rename", oldpath, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("rename", oldpath, False, str(exc))
            return os_error_to_sftp(exc)
    ...
    def posix_rename(self, oldpath: str, newpath: str) -> int:
        try:
            ...
            self._record_op("posix_rename", oldpath, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("posix_rename", oldpath, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("posix_rename", oldpath, False, str(exc))
            return os_error_to_sftp(exc)
    ...
    def mkdir(self, path: str, attr: SFTPAttributes) -> int:
        try:
            ...
            self._record_op("mkdir", path, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("mkdir", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("mkdir", path, False, str(exc))
            return os_error_to_sftp(exc)

    def rmdir(self, path: str) -> int:
        try:
            os.rmdir(self._local(path, follow_symlinks=False))
            self._record_op("rmdir", path, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("rmdir", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("rmdir", path, False, str(exc))
            return os_error_to_sftp(exc)

    def chattr(self, path: str, attr: SFTPAttributes) -> int:
        try:
            SFTPServer.set_file_attr(str(self._local(path)), attr)
            self._record_op("chattr", path, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("chattr", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("chattr", path, False, str(exc))
            return os_error_to_sftp(exc)
    ...
    def readlink(self, path: str):
        try:
            ...
            self._record_op("readlink", path, True)
            return self.jail.to_sftp_path(target_path)
        except PermissionError:
            self._record_op("readlink", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except ValueError:
            self._record_op("readlink", path, False, "Invalid path")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("readlink", path, False, str(exc))
            return os_error_to_sftp(exc)
    ...
    def symlink(self, target_path: str, path: str) -> int:
        try:
            ...
            self._record_op("symlink", path, True)
            return SFTP_OK
        except PermissionError:
            self._record_op("symlink", path, False, "Permission denied")
            return SFTPServer.convert_errno(errno.EACCES)
        except OSError as exc:
            self._record_op("symlink", path, False, str(exc))
            return os_error_to_sftp(exc)
...
class ThreadedSFTPServer:
    ...
    def _serve_client_wrapper(self, client: socket.socket) -> None:
        try:
            serve_client(client, self.host_key, self.auth, self.jail, self.tracker)
        finally:
...
def serve_client(
    ...
    transport.set_subsystem_handler("sftp", SFTPServer, LocalSFTPServer, tracker=tracker)
